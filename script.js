...
function requestPerfumesWhatsApp(){
  const message = encodeURIComponent("Hi! I'd like to shop or request a perfume.");
...
function applySiteSettings(data){
  ...
  if(data.stateDeliveryFees && typeof data.stateDeliveryFees === 'object'){
    STATE_DELIVERY_FEES = { ...STATE_DELIVERY_FEES, ...data.stateDeliveryFees };
  }
...
/* ---------------- Home: "Product Categories" text link row (Glitz-style) ---------------- */
function renderCategoryLinks(){
  const row = document.getElementById('catLinkRow');
  if(!row) return;
  row.innerHTML = CATEGORIES.map(c => `
    <span onclick="selectCategory('${c.key}');showScreen('shop')">${c.label}</span>
  `).join('');
}
...
function openProductDetail(id){
  ...
  document.getElementById('pdKicker').textContent = p.kicker;
...
safeInit('category links', renderCategoryLinks);
