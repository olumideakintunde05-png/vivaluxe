...
let selectedPaymentMethod = 'bank'; // 'bank' | 'paystack'
let paystackPublicKey = null;

function selectPaymentMethod(method){
  selectedPaymentMethod = method;
  document.getElementById('payBankOption').classList.toggle('selected', method === 'bank');
  document.getElementById('payPaystackOption').classList.toggle('selected', method === 'paystack');
  if(method !== 'bank'){
...
function selectBankOption(){
  const wasSelected = selectedPaymentMethod === 'bank';
  ...
  if(wasSelected){
    ...
    document.getElementById('payChevron').classList.add('open');
  }
}
...
function placeOrder(){
  ...
  if(selectedPaymentMethod === 'paystack'){
    ...
    saveOrder({ ...baseOrder, paymentMethod: 'Bank Transfer' });
  }
}
  ...
  db.collection('orders').add(order)
    .then(ref => {
      ...
      document.getElementById('orderConfirmSummary').innerHTML = `
        ...
        <div class="summary-row"><span>Delivery to ${order.customer.state}</span><span>${order.delivery === 0 ? 'Free' : nairaFmt(order.delivery)}</span></div>
