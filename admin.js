...
/* Keys removed via the "Remove" button in Settings, persisted to
   settings/site as `deletedCategories` / `deletedCollections` so a
   removed category/collection doesn't reappear on next load. Only
   custom (type:'tag') collections can be removed — the 3 built-in
   tabs have hardcoded filter logic and stay fixed. */
let deletedCategoryKeys = [];
let deletedCollectionKeys = [];
...
function renderProductCategoryFilters(){
  ...
  if(!chips.find(c => c.key === productCategoryFilter)) productCategoryFilter = 'all';
...
function loadSettingsIntoForm(){
  db.collection('settings').doc('site').get()
    .then(doc => {
      ...
      // Apply any categories removed in a previous session before
      // rendering, so they stay gone.
      deletedCategoryKeys = Array.isArray(currentSiteSettings.deletedCategories) ? currentSiteSettings.deletedCategories.slice() : [];
      deletedCategoryKeys.forEach(key => {
        const idx = ADMIN_CATEGORIES.findIndex(c => c.key === key);
        if(idx > -1) ADMIN_CATEGORIES.splice(idx, 1);
      });
      ...
      // Apply any collections removed in a previous session before
      // rendering, so they stay gone.
      deletedCollectionKeys = Array.isArray(currentSiteSettings.deletedCollections) ? currentSiteSettings.deletedCollections.slice() : [];
      deletedCollectionKeys.forEach(key => {
        const idx = COLLECTION_TABS.findIndex(c => c.key === key && c.type === 'tag');
        if(idx > -1) COLLECTION_TABS.splice(idx, 1);
      });
...
function renderCategoryLabelRows(labels){
  ...
  // ADMIN_CATEGORIES is the live, complete list (base 10 merged with
  // any custom additions, minus anything removed) — see loadSettingsIntoForm.
  wrap.innerHTML = ADMIN_CATEGORIES.map(c => `
    <div style="display:flex;align-items:flex-end;gap:8px;margin-bottom:10px;">
      ...
      <button class="btn-outline" type="button" style="padding:8px 12px;white-space:nowrap;" title="Remove category" onclick="removeCategoryRow('${c.key}')">Remove</button>
    </div>
  `).join('');
}
/* Removes a category from ADMIN_CATEGORIES (client-side) and queues
   it for persistence via deletedCategoryKeys — actually removed from
   Firestore/the live site once "Save Category Names" is clicked.
   Existing products keep whatever category value they already have;
   they just stop matching a live filter/tile until re-tagged. */
function removeCategoryRow(key){
  const cat = ADMIN_CATEGORIES.find(c => c.key === key);
  if(!cat) return;
  const count = adminProducts.filter(p => p.category === key).length;
  const msg = count > 0
    ? `${count} product${count === 1 ? '' : 's'} currently use "${cat.label}". Removing it won't delete those products, but it will disappear from filters and the home page until you save. Remove it?`
    : `Remove "${cat.label}"?`;
  if(!confirm(msg)) return;
  const idx = ADMIN_CATEGORIES.findIndex(c => c.key === key);
  if(idx > -1) ADMIN_CATEGORIES.splice(idx, 1);
  if(!deletedCategoryKeys.includes(key)) deletedCategoryKeys.push(key);
  ...
  if(typeof renderProductCategoryFilters === 'function') renderProductCategoryFilters();
  showSettingsStatus('categoryLabelsSaveStatus', `Removed "${cat.label}" — click Save Category Names to make it live.`, true);
}
...
function saveCategoryLabels(){
  ...
  btn.disabled = true;
  db.collection('settings').doc('site').set({ categoryLabels, customCategories, deletedCategories: deletedCategoryKeys }, { merge: true })
    .then(() => {
...
function renderCollectionLabelRows(labels){
  ...
  wrap.innerHTML = COLLECTION_TABS.map(c => `
    <div style="display:flex;align-items:flex-end;gap:8px;margin-bottom:10px;">
      ...
      ${c.type === 'tag' ? `<button class="btn-outline" type="button" style="padding:8px 12px;white-space:nowrap;" title="Remove collection" onclick="removeCollectionRow('${c.key}')">Remove</button>` : ''}
    </div>
  `).join('');
}
/* Only custom (type:'tag') collections can be removed — "New Arrivals",
   "Best Sellers" and "On Sale" have hardcoded filter logic elsewhere
   and stay fixed (rename-only), same restriction as before this change. */
function removeCollectionRow(key){
  const coll = COLLECTION_TABS.find(c => c.key === key);
  if(!coll || coll.type !== 'tag') return;
  const count = adminProducts.filter(p => Array.isArray(p.collectionTags) && p.collectionTags.includes(key)).length;
  const msg = count > 0
    ? `${count} product${count === 1 ? '' : 's'} tagged into "${coll.label}". Removing it won't untag those products, but the collection tab will disappear until you save. Remove it?`
    : `Remove "${coll.label}"?`;
  if(!confirm(msg)) return;
  const idx = COLLECTION_TABS.findIndex(c => c.key === key);
  if(idx > -1) COLLECTION_TABS.splice(idx, 1);
  if(!deletedCollectionKeys.includes(key)) deletedCollectionKeys.push(key);
  ...
  showSettingsStatus('collectionLabelsSaveStatus', `Removed "${coll.label}" — click Save Collections to make it live.`, true);
}
...
function saveCollectionLabels(){
  ...
  btn.disabled = true;
  db.collection('settings').doc('site').set({ collectionLabels, customCollections, deletedCollections: deletedCollectionKeys }, { merge: true })
    .then(() => {
