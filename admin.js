...
function renderDashboard(){
  ...
  if(lowStockBadge){
    lowStockBadge.textContent = lowStockProducts.length;
    lowStockBadge.style.display = lowStockProducts.length > 0 ? 'inline-flex' : 'none';
  }
  if(lowStockProducts.length === 0){
    lowStockList.innerHTML = `<div class="admin-empty" style="padding:20px;">All products are well stocked.</div>`;
  } else {
    lowStockList.innerHTML = lowStockProducts.map(p => `
      <div class="dash-list-row">
        <span class="dash-list-name">${p.name}</span>
        <span class="dash-stock-pill ${(p.stock ?? 0) === 0 ? 'out' : 'low'}">
          ${(p.stock ?? 0) === 0 ? 'Out of stock' : (p.stock ?? 0) + ' left'}
        </span>
      </div>
    `).join('');
  }
...
function renderAdminProducts(){
  ...
  if(adminProducts.length === 0){
    wrap.innerHTML = `<div class="admin-empty">No products yet — click "Add Product" to create your first one.</div>`;
    return;
  }
...
function setOrderFilter(status){
  orderFilter = status;
  document.querySelectorAll('.admin-filter-btn').forEach(b => b.classList.toggle('active', b.dataset.status === status));
  renderAdminOrders();
}
...
function loadSettingsIntoForm(){
  db.collection('settings').doc('site').get()
    .then(doc => {
      ...
      lowStockThreshold = currentSiteSettings.lowStockThreshold ?? 5;
      document.getElementById('setLowStockThreshold').value = lowStockThreshold;
      renderDashboard();
    })
    .catch(err => {
...
function saveInventorySettings(){
  const threshold = Number(document.getElementById('setLowStockThreshold').value);
  if(!Number.isFinite(threshold) || threshold < 0){
    showSettingsStatus('inventorySaveStatus', 'Please enter a valid number.', false);
    return;
  }
  const btn = document.getElementById('saveInventoryBtn');
  btn.disabled = true;
  db.collection('settings').doc('site').set({ lowStockThreshold: threshold }, { merge: true })
    .then(() => {
      lowStockThreshold = threshold;
      renderDashboard();
      showSettingsStatus('inventorySaveStatus', 'Saved.', true);
    })
    .catch(err => showSettingsStatus('inventorySaveStatus', 'Could not save: ' + err.message, false))
    .finally(() => { btn.disabled = false; });
}
