...
function renderAdminOrders(){
  ...
  if(query){
    list = list.filter(o => {
      ...
      const orderNum = (o.orderNumber || o.id.slice(-6)).toLowerCase();
      return name.includes(query) || phone.includes(query) || orderNum.includes(query) || o.id.toLowerCase().includes(query);
    });
  }
  ...
  wrap.innerHTML = list.map(o => {
    ...
    <div class="admin-order-card">
      <div class="order-top">
        <span class="order-id">#${o.orderNumber || o.id.slice(-6).toUpperCase()}</span>
      ...
      <div class="admin-order-actions">${actions}<button class="btn-outline" onclick="printReceipt('${o.id}')">Print Receipt</button><button class="btn-danger-outline" onclick="adminDeleteOrder('${o.id}')">Delete</button></div>
  ...
  db.collection('orders').doc(orderId).update({ status })
    .then(() => {
      if(status === 'processing'){
        // This is the "Accept" action for a bank-transfer order — the
        // moment it counts as sold, so stock comes off now.
        const order = adminOrders.find(o => o.id === orderId);
        if(order) deductStockForOrder(orderId, order.items);
      }
    })
...
/* ---------------- Stock deduction on approved orders ----------------
   Same logic as script.js's copy of this function (duplicated because
   admin.js and script.js are separate script files, but they both talk
   to the same Firestore project so either one can safely deduct stock).
   Guarded by order.stockDeducted so a double-click can never subtract
...
function deductStockForOrder(orderId, items){
  ...
  }).catch(err => alert('Order status was updated, but stock deduction failed: ' + err.message));
}
function adminDeleteOrder(orderId){
  const o = adminOrders.find(x => x.id === orderId);
  const label = o ? '#' + (o.orderNumber || o.id.slice(-6).toUpperCase()) : 'this order';
  if(!confirm(`Delete ${label} permanently? This cannot be undone.`)) return;
  db.collection('orders').doc(orderId).delete()
    .catch(err => alert('Could not delete order: ' + err.message));
}

/* ---------------- Receipts ----------------
   Opens a new tab with a clean, printable receipt for one order and
   triggers the browser print dialog (which also offers "Save as PDF"). */
function printReceipt(orderId){
  const o = adminOrders.find(x => x.id === orderId);
  if(!o){ alert('Order not found.'); return; }

  const dateStr = o.createdAt?.seconds
    ? new Date(o.createdAt.seconds * 1000).toLocaleString('en-NG', { dateStyle: 'medium', timeStyle: 'short' })
    : '';
  ...
  const itemRows = (o.items || []).map(i => `
    <tr>
      ...
      <td style="text-align:right;">${nairaFmt(i.price)}</td>
      <td style="text-align:right;">${nairaFmt(i.price * i.qty)}</td>
    </tr>`).join('');

  const deliveryLabel = o.deliveryMethod === 'pickup' ? 'Pickup' : 'Delivery';

  const html = `<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Receipt · ${o.orderNumber || orderId}</title>
<style>
  ...
  .totals .row.grand{ font-weight:bold; font-size:15px; border-top:1px solid #333; padding-top:6px; margin-top:6px; }
...
<body>
  <h1>VivaLuxe</h1>
  <div class="sub">Receipt for Order #${o.orderNumber || orderId.slice(-6).toUpperCase()}${dateStr ? ' · ' + dateStr : ''}</div>

  <div class="row"><span>Customer</span><span>${o.customer?.name || 'Guest'}</span></div>
  <div class="row"><span>Phone</span><span>${o.customer?.phone || ''}</span></div>
  ${o.customer?.email ? `<div class="row"><span>Email</span><span>${o.customer.email}</span></div>` : ''}
  <div class="row"><span>${deliveryLabel}</span><span>${o.deliveryMethod === 'pickup' ? '' : (o.customer?.state || '')}</span></div>
  <div class="row"><span>Payment</span><span>${o.paymentMethod || 'Bank Transfer'}${o.paystackReference ? ' · ' + o.paystackReference : ''}</span></div>
  <div class="row"><span>Status</span><span>${ADMIN_ORDER_STATUS_LABELS[o.status] || o.status || 'Pending'}</span></div>
  ...
  <div class="totals">
    <div class="row"><span>Subtotal</span><span>${nairaFmt(o.subtotal || 0)}</span></div>
    <div class="row"><span>${deliveryLabel}</span><span>${o.delivery === 0 ? 'Free' : nairaFmt(o.delivery || 0)}</span></div>
    <div class="row grand"><span>Total</span><span>${nairaFmt(o.total || 0)}</span></div>
  </div>

  <div class="footer">Thank you for shopping with VivaLuxe.</div>
  <div style="text-align:center; margin-top:20px;">
    <button onclick="window.print()" style="padding:8px 18px;font-size:13px;cursor:pointer;">Print / Save as PDF</button>
  </div>
</body>
</html>`;

  const win = window.open('', '_blank');
  if(!win){ alert('Please allow pop-ups to view the receipt.'); return; }
...
/* ---------------- Order alert WhatsApp number ----------------
   Deliberately stored in its own doc (settings/notifications), NOT in
   settings/site — settings/site is read by the public storefront, and
   nothing admin-only should live in a doc a customer's browser loads.
   Give settings/notifications a security rule that only your admin
   UID(s) can read/write, e.g.:
     match /settings/notifications {
       allow read, write: if request.auth != null &&
         exists(/databases/$(database)/documents/admins/$(request.auth.uid));
     }
   The Cloud Function (functions/index.js) reads this same doc with
   admin privileges, so a change here takes effect on the very next
   order — no redeploy needed. */
function loadOrderAlertSettings(){
  db.collection('settings').doc('notifications').get()
    .then(doc => {
      const data = doc.exists ? doc.data() : {};
      document.getElementById('setOrderAlertWhatsapp').value = data.orderAlertWhatsapp || '';
    })
    .catch(err => console.error('Could not load order alert settings:', err));
}
function saveOrderAlertSettings(){
  const orderAlertWhatsapp = document.getElementById('setOrderAlertWhatsapp').value.trim();
  if(!orderAlertWhatsapp){
    showSettingsStatus('orderAlertSaveStatus', 'Enter a WhatsApp number (with country code, e.g. 2349013643713).', false);
    return;
  }
  const btn = document.getElementById('saveOrderAlertBtn');
  btn.disabled = true;
  db.collection('settings').doc('notifications').set({ orderAlertWhatsapp }, { merge: true })
    .then(() => showSettingsStatus('orderAlertSaveStatus', 'Saved — new orders will WhatsApp this number.', true))
    .catch(err => showSettingsStatus('orderAlertSaveStatus', 'Could not save: ' + err.message, false))
    .finally(() => { btn.disabled = false; });
}
...
/* ---------------- Danger Zone: reset all orders ----------------
   Deletes every document in the orders collection and resets the
   order-number counter (counters/orders) back to 0, so the next
   order placed on the storefront starts again at VL001. Everything
   on the Dashboard is computed live from adminOrders (see loadOrders
   further up), so once the orders are gone, revenue, the status
   counts, and Best Sellers all naturally show zero — nothing extra
   to reset there. */
function resetAllOrders(){
  const typed = prompt(
    'This will PERMANENTLY delete every order and reset order numbers back to VL001. This cannot be undone.\n\nType RESET to confirm.'
  );
  if(typed === null) return;
  if(typed.trim().toUpperCase() !== 'RESET'){
    showSettingsStatus('resetOrdersStatus', 'Reset cancelled — you must type RESET exactly.', false);
    return;
  }
  const btn = document.getElementById('resetOrdersBtn');
  btn.disabled = true;
  showSettingsStatus('resetOrdersStatus', 'Deleting all orders…', true);
  db.collection('orders').get()
    ...
    .then(() => db.collection('counters').doc('orders').set({ count: 0 }))
    .then(() => {
      showSettingsStatus('resetOrdersStatus', 'All orders deleted. Order numbers reset to VL001.', true);
    })
    .catch(err => {
      showSettingsStatus('resetOrdersStatus', 'Could not reset orders: ' + err.message, false);
    })
    .finally(() => { btn.disabled = false; });
}
